...
let
  cfg = config.fenir.desktopEnvironment.gaming;
  mainUser = "shiryel"; #config.fenir.mainUser;


in
{
  options.fenir.desktopEnvironment.gaming = {
  ...
  config = mkIf cfg.enable {
    ...
    environment.systemPackages = with pkgs; [
      # NOTE:
      # wakfu needs to be installed with proton 4.11-13
      (steam.override {
        #runtimeOnly = true;
        extraPkgs = pkgs: [ ];
        extraLibraries = pkgs:
