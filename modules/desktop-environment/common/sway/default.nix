...
let
  cfg = config.fenir.desktopEnvironment.sway;
in
{
  options.fenir.desktopEnvironment.sway = {
    enable = mkEnableOption (lib.mdDoc "Sway with a complete desktop environment");
  };
  ...
  config = mkIf cfg.enable {
    ...
    environment.variables = {
      ...
      #LD_PRELOAD = "${pkgs_unstable.glibc}/lib/libc.so.6";
    ...
    fenir.homeManager.toMainUser = {
      ...
      xdg.configFile."sway/config".text =
        ...
        ''
          ...
          for_window [app_id=".telegram-desktop-wrapped"] floating enable
          ...
          exec_always swaymsg 'output ${out_mid} position 0 0'
          exec_always swaymsg 'output ${out_right} position 1920 0'
          exec_always swaymsg 'output ${out_left} mode 2560x1080@60hz'
          exec_always swaymsg 'output ${out_left} transform 270'
          exec_always swaymsg 'output ${out_left} position 3840 0'
          exec_always swaymsg 'output ${out_vr} position 4920 0'

          # xrandr set (to make games work on correct display)
          exec_always ${pkgs.xorg.xrandr}/bin/xrandr --output $(${pkgs.xorg.xrandr}/bin/xrandr | awk '/1920x1080+/ {print $1}' | grep XWAYLAND | head -n 1) --primary
          ...
          exec swaymsg -t get_outputs | ${pkgs.jq}/bin/jq -e '.[1]' && swaymsg 'workspace 12:; exec telegram-desktop'
          # exec discord if 3 monitors
          exec swaymsg -t get_outputs | ${pkgs.jq}/bin/jq -e '.[2]' && swaymsg 'workspace 13:d; exec "${pkgs.foot}/bin/foot -e ${pkgs.btop}/bin/btop"; layout toggle split; exec discord'
          ...
          exec swaymsg 'for_window [title=.*] max_render_time 8'
          exec swaymsg 'output * max_render_time 8'
