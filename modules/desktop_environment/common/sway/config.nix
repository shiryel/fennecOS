...
let
  ...
  lock = "${pkgs.swaylock}/bin/swaylock -f -i ~/.config/sway/lockscreen";
  logout = "${pkgs.sway}/bin/swaynag -t warning -m 'Power Menu Options' -b 'Logout' 'swaymsg exit' -b 'Restart' 'shutdown -r now' -b 'Shutdown'  'shutdown -h now' --background=#002b33DD --button-background=#0077b3DD --button-text=#FFFFFF --button-border-size=0 --text=#FFFFFF --border-bottom-size=0 --button-margin-right=10";
  printclip = "${pkgs.slurp}/bin/slurp | ${pkgs.grim}/bin/grim -g - - | ${pkgs.wl-clipboard}/bin/wl-copy";
  printscreen = ''${pkgs.grim}/bin/grim -t png -l 9 -o $(${pkgs.sway}/bin/swaymsg -t get_outputs -r | ${pkgs.jq}/bin/jq -r '.[] | select(.focused == true) | .name') - > ~/Downloads/screenshot.png && ${pkgs.libnotify}/bin/notify-send -t 1000 "screenshot saved on ~/Downloads'';
in
pkgs.writeTextFile {
  name = "sway-config";
  text = ''
    ...
    # dangerous - 2/bottom
    bindsym $mod+Shift+k exec ${logout}
    ...
    exec swaymsg -t get_outputs | ${pkgs.jq}/bin/jq -e '.[2]' && swaymsg 'workspace 13:d; exec "${terminal} -e ${pkgs.btop}/bin/btop"; layout toggle split; exec ${pkgs.discord}/bin/discord'
    ...
    exec swaymsg 'output ${out_mid} adaptive_sync on'
